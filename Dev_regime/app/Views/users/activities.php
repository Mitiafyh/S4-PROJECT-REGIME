    ...
    <div class="min-h-screen flex overflow-hidden">
        ...
        <aside class="w-64 bg-white/60 backdrop-blur-xl border-r border-stone-200/60 flex-col hidden md:flex sticky top-0 h-screen z-10">
            ...
            <nav class="flex-1 px-4 py-8 space-y-1 overflow-y-auto">
                ...
                <a href="<?= site_url('users/activities') ?>" class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300 text-sm font-medium bg-stone-800 text-white shadow-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <?php if(!empty($activitiesList)): ?>
                        <?php foreach($activitiesList as $activity): ?>
                            <?php 
                                $isSelected = in_array($activity->id, $userActivityIds ?? []);
                                $images = [
                                ...
                                $image = $images[($activity->id - 1) % count($images)] ?? $images[0];
                                $intensite = $activity->depense_calorique > 400 ? 'Intense' : ($activity->depense_calorique > 250 ? 'Modérée' : 'Douce');
                            ?>
                            <div class="group cursor-pointer flex flex-col h-full bg-white rounded-3xl p-4 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] border <?= $isSelected ? 'border-emerald-200 bg-emerald-50' : 'border-stone-100' ?> hover:shadow-[0_12px_40px_-4px_rgba(0,0,0,0.06)] transition-all duration-500">
                                ...
                                <div class="px-2 flex-1 flex flex-col">
                                    ...
                                    <h4 class="text-xl font-medium text-stone-800 mb-2"><?= esc((string)($activity->type ?? 'Activité')) ?></h4>
                                    <p class="text-stone-500 text-sm flex items-center gap-2 mt-auto">
                                        ...
                                        <?= intval($activity->duree) ?> min | Dépense: <?= intval($activity->depense_calorique) ?> cal
                                    </p>
                                </div>
                            </div>
                        <?php endforeach; ?>
