<?php
$codesList = $codes ?? [];
$creditAmountValue = (int) ($creditAmount ?? 50);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>NutriFlow - Admin Codes Promo</title>
    ...
    <div class="min-h-screen flex overflow-hidden">
        ...
        <aside class="w-64 bg-stone-950 border-r border-stone-800 flex-col hidden md:flex sticky top-0 h-screen z-10">
            ...
            <nav class="flex-1 px-4 py-8 space-y-1 overflow-y-auto">
                ...
                <a href="<?= site_url('admin/codes') ?>" class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300 text-sm font-medium bg-stone-800 text-white shadow-md">
        ...
        <main class="flex-1 overflow-y-auto h-screen relative">
            <div class="max-w-4xl mx-auto px-6 md:px-12 py-8 md:py-12">
                ...
                <div class="flex justify-between items-center mb-12">
                    <header>
                        <h2 class="text-3xl font-medium text-white mb-2">Codes Promo</h2>
                        <p class="text-stone-400">Gérez les codes pour créditer les portefeuilles utilisateurs.</p>
                    </header>
                </div>

                <div class="bg-stone-900 border border-stone-800 rounded-2xl p-6 mb-8 shadow-xl shadow-black/20">
                    <h3 class="text-lg font-medium text-white mb-6">Créer un nouveau code</h3>
                    <form method="POST" action="<?= site_url('admin/codes/create') ?>" class="flex gap-4">
                        <div class="flex-1">
                            <label class="block text-xs font-medium text-stone-500 mb-2">Code</label>
                        ...
                        <div class="flex items-end">
                            <button type="submit" class="px-6 py-3 bg-white text-stone-900 rounded-xl font-medium hover:bg-stone-200 transition-colors">
                                Ajouter
                            </button>
                        </div>
                    </form>
                        ...
                        <tbody class="divide-y divide-stone-800">
                            <?php if (!empty($codesList)): ?>
                                <?php foreach ($codesList as $code): ?>
                                    <?php $isActive = ($code['status'] ?? '') === 'active'; ?>
                                    <tr class="hover:bg-stone-800/30 transition-colors">
                                        <td class="px-6 py-4 font-mono text-stone-200 tracking-wider <?= $isActive ? '' : 'line-through text-stone-500' ?>">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="px-2.5 py-1 <?= $isActive ? 'bg-emerald-500/10 text-emerald-400' : 'bg-stone-800 text-stone-400' ?> text-xs rounded-full">
                                                <?= $isActive ? 'Actif' : 'Utilise' ?>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            <a href="<?= site_url('admin/codes/delete/' . ($code['id'] ?? 0)) ?>" class="text-rose-400 hover:text-rose-300" onclick="return confirm('Supprimer ce code ?');">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td class="px-6 py-4 text-stone-500" colspan="4">Aucun code en base.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
